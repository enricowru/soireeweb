...
# ------------------------------------------------------------------
# Custom Event Package images (based on pax threshold)
# ------------------------------------------------------------------

def get_custom_event_package_images(num_pax):
    """Return package image list for custom (user-defined) events.

    The logic is simple: if the number of guests is 100 or below we return the
    "Below 100 Pax" package poster; otherwise we return the "Above 100 Pax"
    poster.  Replace the public IDs below with the real Cloudinary public IDs
    of the posters you have uploaded.
    """
    # Replace these public IDs with your actual Cloudinary uploads
    below_public_id = 'Others_package_akx2zo'
    above_public_id = 'others_package2_zazqff'

    if num_pax <= 100:
        url = cloudinary.utils.cloudinary_url(below_public_id, secure=True)[0]
        return [('Below 100 Pax', url)]
    else:
        url = cloudinary.utils.cloudinary_url(above_public_id, secure=True)[0]
        return [('Above 100 Pax', url)]
    ...
    if request.method == 'POST':
        ...
        if planner_state == 'date_input':
            ...
            if is_valid:
                ...
                if request.session.pop('pending_finish', False):
                    bot_response = {'sender':'bot','text':'Date updated successfully.','show_finish_options':True}
            ...
            try:
                ...
                pending_edit = request.session.get('pending_finish', False)
                if pending_edit and ((planner_data.get('is_custom_event', False) and num_pax>0) or (not planner_data.get('is_custom_event', False) and num_pax in valid_pax_options)):
                    ...
                    planner_state = 'food_select_finish'
                    request.session.pop('pending_finish', None)
                    # Short-circuit the rest of this state handler
                elif planner_data.get('is_custom_event', False):
                    if num_pax > 0:
                        # Accept any positive number for custom events
                        planner_data['number_of_pax'] = num_pax
                        if request.session.pop('pending_finish', False):
                            bot_response = {'sender':'bot','text':'Number of guests updated successfully.','show_finish_options':True}
                            planner_state = 'food_select_finish'
                        else:
                            bot_response = {
                                'sender': 'bot',
                        ...
                        bot_response = {
                            'sender': 'bot',
                            'text': "Please enter a valid positive number of guests.",
                            'show_pax_input': True,
                            'pax_options': []
                        }
                elif num_pax in valid_pax_options:
                    planner_data['number_of_pax'] = num_pax
            ...
            if is_valid:
                ...
                if request.session.pop('pending_finish', False):
                    bot_response = {'sender':'bot','text':'Venue updated successfully.','show_finish_options':True}
                    planner_state = 'food_select_finish'
                else:
                    # Ask for color palette next
                    bot_response = {
                        'sender': 'bot',
                        'text': "Great! Please choose a color palette for your event using the picker below.",
            ...
            if is_valid:
                ...
                if request.session.pop('pending_finish', False):
                    bot_response = {'sender':'bot','text':'Color palette updated successfully.','show_finish_options':True}
                    planner_state = 'food_select_finish'
                else:
            ...
            if user_message.lower() in ['yes', 'y']:
                if request.session.pop('pending_finish', False):
                    # User confirmed new theme during change flow, return to finish panel
                    bot_response = {
                        'sender': 'bot',
                    ...
                    if planner_data.get('is_custom_event', False):
                        # For custom events we still want to offer packages, but the poster
                        # depends on the pax (100 or below / above 100).
                        num_pax = planner_data.get('number_of_pax', 0)
                        package_images = get_custom_event_package_images(num_pax)

                        if package_images:
                            planner_data['package_images'] = package_images

                            selected_label, selected_url = package_images[0]
                            # Persist for summary later
                            planner_data['package_image'] = selected_url
                            planner_data['package_image_label'] = selected_label
                            ...
                            # Fallback straight to food menu (should not happen)
            ...
            lower_cmd = user_message.lower().strip()

            change_map = {
                'change date': ('date_input', {'sender':'bot','text':'Okay, what new date would you like?','show_date_input':True}),
                'change number of pax': ('number_of_pax', {'sender':'bot','text':'Okay, enter the new number of guests (pax).','show_pax_input':True,'pax_options': get_pax_options(planner_data.get('event_type','').lower())}),
                'change venue location': ('venue_location', {'sender':'bot','text':'Okay, what is the new venue or location?','show_venue_input':True}),
                'change color': ('color_palette', {'sender':'bot','text':'Okay, please pick a new color palette:','show_color_picker':True}),
                'change theme': ('theme_choice', {'sender':'bot','text':'Sure, let\'s choose a new theme category:','show_theme_category_buttons':True,'theme_category_options': get_all_categories()}),
                'change event type': ('event_type', {'sender':'bot','text':'Sure, what new type of event would you like to plan?','show_event_buttons':True}),
            }

            if lower_cmd in change_map:
                # Set target state and prompt
                planner_state, bot_prompt = change_map[lower_cmd]
                bot_response = bot_prompt
                # Mark that we should return to finish panel afterwards
                request.session['pending_finish'] = True

            elif lower_cmd in ['finish', 'done']:
                # Prepare summary
                summary_lines = []
                ...
                if not planner_data.get('is_custom_event', False):
                    ...
                    color_hex = planner_data.get('color_palette', 'N/A')
                    if isinstance(color_hex, str) and color_hex.startswith('#') and len(color_hex) in [4,7]:
                        color_box_html = f"<span style=\"display:inline-block;width:20px;height:20px;background:{color_hex};border:1px solid #000;margin-left:6px;\"></span>"
                        summary_lines.append({'type': 'text', 'value': f"Color palette: {color_hex} {color_box_html}"})
                    else:
                        summary_lines.append({'type': 'text', 'value': f"Color palette: {color_hex}"})
                ...
                # Build structured HTML summary to mimic contract sheet
                event_type_disp = planner_data.get('event_type', 'N/A').title()
                event_date_disp = planner_data.get('event_date', 'N/A')
                num_pax_disp = planner_data.get('number_of_pax', 'N/A')
                venue_disp = planner_data.get('venue_location', 'N/A')
                color_disp = planner_data.get('color_palette', 'N/A')
                theme_disp = planner_data.get('theme', '').title() if planner_data.get('theme') else ''
                # Leave separate fields for color motif and theme
                
                package_disp = planner_data.get('package_choice', 'N/A')

                dishes_disp = ', '.join(planner_data['food_selection'].get('dishes', [])) if planner_data.get('food_selection') else ''
                pasta_disp = ', '.join(planner_data['food_selection'].get('pasta', [])) if planner_data.get('food_selection') else ''
                drinks_disp = ', '.join(planner_data['food_selection'].get('drinks', [])) if planner_data.get('food_selection') else ''

                summary_html = f"""
                <h3 style='margin-top:0;'>Event Information</h3>
                <table class='info-table' style='width:100%; font-size:0.95em;'>
                    <tr><td style='width:35%;'>Date and Time of Event:</td><td>{event_date_disp}</td></tr>
                    <tr><td>Type of Event:</td><td>{event_type_disp}</td></tr>
                    <tr><td>Number of Guests:</td><td>{num_pax_disp}</td></tr>
                    <tr><td>Venue or Location of Event:</td><td>{venue_disp}</td></tr>
                    <tr><td>Color Motif of Event:</td><td>{color_disp}</td></tr>
                    <tr><td>Theme of Event:</td><td>{theme_disp or 'N/A'}</td></tr>
                    <tr><td>Name of Celebrant:</td><td>&nbsp;</td></tr>
                </table>

                <h3 style='margin-top:18px;'>Package and Food Inclusion</h3>
                <p><strong>Package:</strong> {package_disp}</p>
                <ul style='margin-left:18px;font-size:0.95em;'>
                    <li><strong>Dishes:</strong> {dishes_disp}</li>
                    <li><strong>Pasta:</strong> {pasta_disp}</li>
                    <li><strong>Drinks:</strong> {drinks_disp}</li>
                    <li>Rice: Steamed Rice</li>
                    <li>Dessert: Dessert Buffet</li>
                </ul>
                """
            ...
            if lower_cmd == 'change event type':
                # Clear dependent planner_data so the user can re-enter them
                for key in ['event_type','is_custom_event','number_of_pax','venue_location','color_palette','theme','theme_images','package_letter','package_choice','package_image','package_images','food_selection']:
                    planner_data.pop(key, None)
                # Also reset any intermediate flags
                request.session.pop('pending_finish', None)

            if lower_cmd == 'change theme':
                for key in ['theme','theme_images','current_category']:
                    planner_data.pop(key, None)
                request.session['pending_finish'] = True

            # ----- Custom flow for "change food menu" button -----
            if lower_cmd == 'change food menu':
                # Clear previous food selections
                planner_data.pop('food_selection', None)

                # Prepare rules text based on current package
                pkg_letter = planner_data.get('package_letter')
                rules = PACKAGE_RULES.get(pkg_letter, {'dishes':3,'pasta':1,'drinks':1,'rice':True,'dessert':True})
                rules_text = (
                    f"Based on Package {pkg_letter or '?'} rules, please review the menu and press OK when you are ready to make your selections.\n\n"
                    f"You can choose:\n<ul><li>{rules['dishes']} dishes</li><li>{rules['pasta']} pasta</li><li>{rules['drinks']} drink</li></ul>\n"
                    "Steamed Rice and Dessert Buffet are already included."
                )

                bot_response = {
                    'sender':'bot',
                    'text': rules_text,
                    'images': get_food_menu_images(),
                    'show_food_menu_section': True
                }
                request.session['pending_foodmenu_change'] = True  # marker, though not strictly needed
                planner_state = 'food_menu_review'
                processed_finish_branch = True

            # Custom flow for change package ---------------------------------
            if lower_cmd == 'change package':
                # reset package and food selections
                for key in ['package_letter','package_choice','package_image','package_image_label','food_selection']:
                    planner_data.pop(key, None)

                # get package images
                event_type_raw = planner_data.get('event_type','').lower()
                event_key_map = {'kiddie party':'kiddie','birthday party':'birthday','wedding':'wedding','christening':'christening'}
                event_key = event_key_map.get(event_type_raw, event_type_raw)
                package_images = get_event_package_images(event_key)
                planner_data['package_images'] = package_images

                # pick display image
                sel_label, sel_url = (package_images[0] if package_images else (None,None))
                if package_images:
                    current_pax = planner_data.get('number_of_pax')
                    def _pax(lbl):
                        import re
                        m = re.search(r'(\d+)\s*Pax', lbl, re.IGNORECASE); return int(m.group(1)) if m else None
                    match = [img for img in package_images if _pax(img[0])==current_pax]
                    if match:
                        sel_label, sel_url = match[0]

                request.session['pending_package_change'] = True

                bot_response = {
                    'sender':'bot',
                    'text': f"You selected: {sel_label}" if sel_label else "Please choose a package:",
                    'package_image_selected': sel_url,
                    'package_image_label': sel_label
                }
                planner_state = 'package_letter_choice'
                # skip rest of processing by setting a flag
                lower_cmd = ''
            # ----------------------------------------------------------------
...
def handle_state_transition(request, current_state, user_message, planner_data):
    ...
    if current_state == 'date_input':
        try:
            date_obj = datetime.strptime(user_message, '%Y-%m-%d').date()
            # Disallow past dates
            if date_obj < datetime.now().date():
                return False, "Please choose a date that is today or in the future.", None
