...
def booking_details_api(request, booking_id):
    """API endpoint to get booking details for the modal - Enhanced to match user API"""
    try:
        booking = get_object_or_404(BookingRequest, id=booking_id)
        
        # Import the user API logic for consistency
        from .user import format_color_motif_display
        import json
        
        # Color names mapping (same as in user API)
        color_images_raw = {
        ...
        # Food names mapping (same as in user API)
        food_names = {
            # Beef dishes
        ...
        print(f"Error in admin booking_details_api: {error_details}")
